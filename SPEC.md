...
| **Collection** | 문헌 묶음(선반) 1계층 + 순서 — 문헌 위치는 1곳 | Content(조직화) | 유저 (카탈로그 선반은 1회 시드) |
...
| # | 기능 | 동작 |
|---|---|---|
| O1 | 폴더(컬렉션) | **2.3.0 구현, 2.3.1 UI 정리** — `parentId` 트리로 임의 깊이 저장, 폴더 진입·breadcrumbs·이름변경·삭제·문헌 이동. 폴더는 카드 안 아이콘이 아니라 탭 달린 폴더 실루엣 자체로 표시한다. 생성 진입점은 홈 toolbar의 단일 **추가** 메뉴이며, 첫 UI의 새 폴더 생성은 루트 아래 2단계까지. 삭제 시 문헌·하위 폴더를 부모로 승격해 보존. 카탈로그 컬렉션은 최초 1회 시드 후 **완전 사용자 소유** (`mundok-v3/collections`) |
...
| 선반 소유 모델 (2026-08-01 추가) | 카탈로그 선반 **1회 시드 → 사용자 소유** (시드는 유저 공간 실존 문헌만·빈 선반 제외), 문헌 위치 1곳(굿노트형 폴더-파일), '내 문헌'·'미분류' 통합, 열린 선반 시각(널판·접힘 색칩) — O1 | 읽기 전용 카탈로그 선반 병존(빼기 불가 선반 발생) · 다중 선반 중복 배치 · 폴더 진입형 2단 탐색(홈 런처 역할 약화) |
