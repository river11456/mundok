...
| # | 기능 | 동작 |
|---|---|---|
| O1 | 선반(컬렉션) | **2.2.0 구현** — 생성·이름변경·삭제(문헌은 미분류로), 문헌 이동(상세 오버레이 '선반 이동' — 위치 1곳, 미분류 = 선반 밖). 카탈로그 선반은 최초 1회 시드 후 **완전 사용자 소유** (`mundok-v3/collections`) |
| O2 | 참고문헌 관계 | 구 `_groups.refs` → 카탈로그 컬렉션 메타(`_collections.json`)로 이전 — 계속 카탈로그 소유. 선반 명시 배치가 자식 숨김보다 우선 |
| O3 | 홈 정렬 | 사용자 선반 순서 → 미분류(카탈로그 order → 생성순). 구 '내 문헌' 선반은 2.2.0에서 미분류로 통합 (새 문헌·문헌 받기 타일 상주) |
