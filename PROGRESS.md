...
## 📌 현재 상태 (2026-07-19) — 콘텐츠 개방 3종: 셀 선택 저작·문헌 추가·카탈로그 (v1.15.0~v1.17.0) — ⭐ 다음 세션은 여기부터

사용자 논의로 설계 확정 후 순차 구현. **모두 커밋 완료, 푸시 전.**
...
| v1.17.0 | **문헌 카탈로그** — `catalog/*.json` → 빌드 시 `dist/catalog/`(index.json 포함, build-catalog vite 플러그인) → Pages 동일 origin 배포. 홈 '문헌 받기' 모달: 받기·업데이트·설치됨 상태, SW 캐시는 ?v= 쿼리 버스팅. 설치본 = source 메타 붙은 user-doc, 업데이트는 카드 id 유지 통째 교체(학습기록 보존). lint가 카탈로그도 검사. **현재 카탈로그는 비어 있음** — 등록은 파일 추가+push |
...
- ⚠ **잔여 확인**: 아이패드 실기기 — **셀 선택**(홀드·가로 긋기·펜슬, 이전 세션의 롱프레스 확인 항목은 이 교체로 대체됨), 새 문헌 마법사 터치, 카탈로그 모달. 데스크톱 육안 — 선택 하이라이트·버블 위치
- 다음 후보: 카탈로그에 첫 문헌 등록(써머리 기반), 홈 화면 설치 유도 온보딩, UI 디테일 계속

---

## 📌 이전 상태 (2026-07-18 밤) — 반응형·터치 재개: 5·6단계(C2·C3) + 폰 지원 (v1.11.2~v1.14.0)
...
## 📌 이전 상태 (2026-07-18) — 글자 셀 렌더링 + 문법 UI 개선 (v1.8.0~v1.11.0)
