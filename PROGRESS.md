...
## 📌 현재 상태 (2026-07-18) — 文 아이콘 확장 메뉴 (v1.11.0)

문법 컨트롤을 文 아이콘 하나로 통합 (사용자 지시):

- **文 아이콘 클릭 → 확장 메뉴**(`.gram-menu`): `문법 표시`(G 키캡 힌트)·`문법 편집` 두 항목, 각각 토글·활성 표시(`.gram-item.on`). 아이콘은 표시/편집 중 파란 활성
- 하단 `文法` 필 제거(메뉴의 "문법 표시"와 중복) — G 단축키·도움말은 유지
- 닫기: 항목 선택·바깥 클릭(닫은 뒤 클릭 액션 계속 처리)·Esc·카드 이동(`resetGrammarView`)
- 검증: 테스트 49건·빌드 통과, gram-pill 잔재 0

---

## 📌 현재 상태 (2026-07-18) — 문법 카드별 수동 표시 + 文 아이콘 (v1.10.0)

v1.9.0 피드백 2건 반영:

- **문법 = 카드 단위 수동 켜기 (B안)**: `resetGrammarView()`(state.ts) — 카드가 바뀌는 모든 시점(순차 이전/다음·안키 rate·드릴다운 진입/복귀·학습 시작·홈)에 보기·편집 자동 OFF. 같은 카드 안에서 Space 뒤집기는 유지
- **편집 아이콘**: 마커 SVG(연필과 혼동) → **文 글자**(해서) — 하단 文法 필과 어휘 통일
- 검증: 테스트 49건·빌드 통과

---

## 📌 현재 상태 (2026-07-18) — 문법 UI 성격별 분리 (v1.9.0)

우측 상단 `文法|보기|편집` 세그먼트(토글 2개가 라디오처럼 보이던 문제) 폐지 — 인터뷰 A안:

- **보기 = 학습 토글** → 카드 하단 `文法` 필(`.gram-pill`, 순차: Space 옆 / 안키: 난이도 아래) + **단축키 G** 신설(문장 카드, 도움말 등재)
- **편집 = 저작 도구** → 우측 상단 카드 도구(연필·휴지통) 사이 마커+밑줄 아이콘(`icon-btn.on` 활성 상태), 동작·저장은 기존 그대로
- v1.8.1: 음-밑줄 간격 8px + 음 표시 중 `rd-on` 줄 간격 `calc(1em+46px)` 확장 (사용자 피드백)
- 검증: 테스트 49건·빌드 통과, `.seg` 잔재 0. ⚠ 육안 확인: 필 위치·아이콘 인지성

---

## 📌 현재 상태 (2026-07-18) — 글자 셀 렌더링 통일 (v1.8.0)

학습 카드 본문 구조 개선 — 명세·결정 기록은 **`design/char-cell.md`** (사용자 인터뷰로 확정, R1~R9):

- **문제**: 음·드릴 밑줄·문법 표시가 문자열에 각기 다른 방식으로 덧칠 → ① 전체 길이 일치 시에만 글자별 음 ② 뒤집을 때 배치 흔들림 ③ 드릴 구간 안팎 음 위치 상이 ④ 드래그에 음 섞임
- **`src/reading-align.ts` 신설**: 한자에만 1:1 독음 정렬(공백·부호·현토 통과), 실패 시 null — 렌더러·lint 단일 출처 (lint가 `.ts`를 직접 import — Node 22.18+ 기본 type stripping)
- **`render.ts` 통일**: `tokenizeHighlights`/`annotatedFront`/`renderGrammarSentence`(ruby·inline-block·상태기계 3중 구조) 폐지 → `renderFront()` 글자 셀(`.cc`) 단일 경로. 음(`.cc-rd`)·SVO 레이블(`.cc-svo`)은 absolute → 앞뒷면 줄바꿈·가로 배치 동일, 음 토글 시 밑줄 부동. 문법 편집 그리드도 같은 표기(`.cc-rd-e`)
- **R6**: char 카드 하단 예약(`.char-hold`) — 뒤집어도 대형 한자 부동. **R7**: 카드 추가 드래그가 선택 범위를 복제해 `.cc-rd`/`.cc-svo` 제거 후 읽음 — 음 절대 미포함
- **R5 lint**: 독음 정렬 실패 WARN (한자 2자 이상만 — 1자 카드 훈+음 관행 허용). 현 데이터 0건
- 검증: 테스트 49건(신규 15) 통과, tsc·빌드 통과, 번들에 셀 클래스 5종 확인, ruby 잔재 0. ⚠ **브라우저 육안 확인 필요** — char-cell.md 4절 검증 기준 7항목(뒤집기 배치 불변·드릴 안팎 음 높이·드래그 등)
- 잔여 재량(5절): 음 표시 시 줄 간격 수치·`.char-hold` 150px 등 미세 조정은 육안 확인 후
...
### 방향 결정 (2026-07-19, 사용자) — ⭐ 다음 세션은 여기부터
