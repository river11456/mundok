...
기기를 바꿀 때는 같은 메뉴의 "백업 파일에서 가져오기"로 복원합니다. 현재 브라우저 데이터를 백업 내용으로 덮어쓰므로 가져오기 전에도 백업해 두세요. 복원 실패 시 데이터 보존과 입력 검증에 [알려진 문제](docs/recovery.md)가 있어 정상 백업은 별도로 보관하고, 오류가 나면 덮어쓰기를 반복하지 마세요.
